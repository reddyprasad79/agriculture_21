...
(function () {
  ...
  function getDashboardHref(role) {
    return role === "farmer" ? "farmer-dashboard.html" : "agri-business-dashboard.html";
  }
  ...
  if (session && session.role && navLinks) {
    const dashboardHref = getDashboardHref(session.role);
    const hasDashboardLink = Array.from(navLinks.querySelectorAll("a")).some(function (link) {
      return link.getAttribute("href") === dashboardHref;
    });
  ...
  if (session && session.role && navActions) {
    const dashboardHref = getDashboardHref(session.role);
    navActions.innerHTML = [
      '<a class="btn btn-light" href="' + dashboardHref + '">Dashboard</a>',
      '<button class="btn btn-gold" type="button" id="publicLogoutBtn">Sign Out</button>'
    ].join("");
  }
  ...
  const publicLogout = document.getElementById("publicLogoutBtn");
  if (publicLogout) {
    publicLogout.addEventListener("click", function () {
